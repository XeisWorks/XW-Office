...
class WixProductsClient:
    ...
    def brand_endpoints_selfcheck(self) -> dict[str, Any]:
        """Probe Wix brand query endpoints and report which ones are reachable."""
        result: dict[str, Any] = {
            "has_credentials": self.has_credentials(),
            "query_ok": False,
            "preferred_query_endpoint": "",
            "reachable_query_endpoints": 0,
            "checks": [],
        }
        ...
        query_payload = {"query": {"paging": {"limit": 1}}}
        with httpx.Client(timeout=_TIMEOUT) as client:
            for endpoint in self._brand_endpoint_candidates("/query"):
                status_code = 0
                ok = False
                ...
                message = ""
                try:
                    resp = client.request("POST", endpoint, headers=headers, json=query_payload)
                    ...
                    if resp.status_code < 400:
                        ...
                        if isinstance(data, dict):
                            ...
                            if not isinstance(raw, list):
                                raw = data.get("results")
                        parsed_list = isinstance(raw, list)
                        if parsed_list:
                            message = f"OK ({len(raw)} Brands gelesen)"
                        else:
                            message = "OK (Antwort ohne bekannte Brand-Liste)"
                    else:
                ...
                if ok:
                    reachable_query_endpoints += 1
                    if not preferred_query_endpoint and parsed_list:
                        preferred_query_endpoint = endpoint
        ...
        if not preferred_query_endpoint:
            for item in checks:
                if bool(item.get("ok")):
                    preferred_query_endpoint = str(item.get("endpoint") or "")
                    break

        result["query_ok"] = bool(preferred_query_endpoint)
        result["preferred_query_endpoint"] = preferred_query_endpoint
        result["reachable_query_endpoints"] = reachable_query_endpoints
