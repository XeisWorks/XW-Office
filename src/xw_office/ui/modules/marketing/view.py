...
from xw_office.services.ideas.store import IdeaEntry
from xw_office.services.ideas.stores import MarketingIdeasStore
...
class MarketingView(QWidget):
    ...
    def __init__(self, container: Container, parent: QWidget | None = None) -> None:
        ...
        self._store: MarketingIdeasStore | None = None
        ...
        self._detail.setPlainText("Ideen werden geladen...")
        self._load_async()
    ...
    def _load_async(self) -> None:
        ...
        def job() -> tuple[MarketingIdeasStore, list[IdeaEntry]]:
            store = self._container.resolve(MarketingIdeasStore)
    ...
    def _on_loaded(self, payload: object) -> None:
        if not isinstance(payload, tuple) or len(payload) != 2:
            return
        store, ideas = payload
        if not isinstance(store, MarketingIdeasStore) or not isinstance(ideas, list):
            return
        self._store = store
    ...
    def _persist_async(self, operation: Callable[[MarketingIdeasStore], None]) -> None:
    ...
    def _on_store_error(self, exc: Exception) -> None:
        QMessageBox.warning(self, "Marketing", f"Speichern/Laden fehlgeschlagen:\n{exc}")
